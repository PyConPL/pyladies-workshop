...
3) Create a new route (a new function in ``flaskr.py``) "/funny-cat" which points to funny-cat.html template (there is no need to connect to database here). Create basic HTML with a picture (_help) of a funny cat in the middle of the page (from here_)! Place file of a funny cat in ``static`` folder. Make sure that about extends ``layout.html``. 
